...
HttpLiveStreamingWriter._escapeAttribute = function(attribute, value) {
  ...
  var dontQuote = ['RESOLUTION', 'TYPE'].indexOf(attribute) > -1;
...
HttpLiveStreamingWriter.prototype.media = function(type, attributes) {
  attributes.type = type;
  this.comment('EXT-X-MEDIA:' + this.constructor._escape(attributes));
};
...
HttpLiveStreamingWriter.prototype.iframePlaylist = function(uri, attributes) {
  attributes.uri = uri;
  this.comment('EXT-X-I-FRAME-STREAM-INF:' + this.constructor._escape(attributes));
};
